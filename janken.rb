...
class Janken
  def pon(player_hand, enemy_hand)#（）内は引数。仮の分かりやすい名前を付ける。自分と相手の手の情報の順番。
    # 変数（配列）「janken」に["グー", "チョキ", "パー"]を代入します。
    janken = ["グー", "チョキ", "パー"]
    #「相手の手は#{相手の手}です。」と出力させます。
    puts "相手の手は#{janken[enemy_hand]}です。"#[enemy_hand]の中身には0,1,2が来てほしい。
...
class GameStart
    ...
    def self.jankenpon
      ...
      janken = Janken.new#じゃんけんする場を提供している
      ...
      next_game = janken.pon(player.hand, enemy.hand)#next_gameを定義して、実行される。ponは引数で2つ必要。
      end
    end
end
